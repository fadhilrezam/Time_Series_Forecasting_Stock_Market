...
def preprocess_dataset(df):
    try:
        ...
        return df, df_train, df_test, exog_train, exog_test, exog_future
...
def store_dataframe(df, df_train, df_test, exog_train, exog_test):
    ...
    try:
        ...
        if os.path.exists(folder_path):
            ...
            exog_future.to_csv(os.path.join(folder_path, 'exog_future.csv'), index = True)
            logging.info(f'Preprocessed Dataframe Successfully stored in {folder_path}')
    except Exception as e:
        logging.error(CustomException(e,sys))
    
if __name__ == '__main__':
    df = load_dataset()
    df_cleaned, df_train, df_test, exog_train, exog_test, exog_future = preprocess_dataset(df)
